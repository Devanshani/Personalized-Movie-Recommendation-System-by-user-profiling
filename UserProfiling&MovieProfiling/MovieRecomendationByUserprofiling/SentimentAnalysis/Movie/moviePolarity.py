import pypyodbc;
import SentimentAnalysis.Movie.movieClassifier as WSD
import  os.path
    ...
    for row in cursor.fetchall():
        ...
        # print(row[0])
            ...
            if polarity == 'pos':
                ...
                cursor.execute("UPDATE ReviewsNew SET PolarityConfidence='"+str(confidencepos)+"' WHERE ReviewId =" + str(id))
                cursor.execute("UPDATE ReviewsNew SET PolarityClassify='" + str(polarity) + "' WHERE ReviewId =" + str(id))
                cursor.commit()
            else:
                count1=count1+1
                confidenceneg=(-100)*confidence
                cursor.execute("UPDATE ReviewsNew SET PolarityConfidence='" +str(confidenceneg) + "' WHERE ReviewId =" + str(id))
                cursor.execute("UPDATE ReviewsNew SET PolarityClassify='" + str(polarity) + "' WHERE ReviewId =" + str(id))
                cursor.commit()
