...
if __name__ == "__main__":
    ...
    for id in range(1,51):
        cursor.execute("SELECT PolarityConfidence FROM UserReviews WHERE userId="+ str(id))
        count=0
        countarray=[]
        print(id)
    ...
    # print(countarray)
        ...
        if (len(countarray) != 0):
            ...
            cursor.execute("UPDATE UserInfo SET AvReviewPolarity ='" + str(avg) + "'WHERE Id =" + str(id))
            cursor.commit()
        else:
            cursor.execute("UPDATE UserInfo SET AvReviewPolarity ='" + str(0) + "'WHERE Id =" + str(id))
            cursor.commit()
    cursor.close()