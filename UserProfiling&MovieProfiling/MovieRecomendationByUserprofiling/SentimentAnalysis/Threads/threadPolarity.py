import pypyodbc;
import SentimentAnalysis.Threads.threadClassifier as WSD
import  os.path
...
if __name__ == "__main__":
    ...
    cursor = con.cursor()
    cursor.execute("SELECT PreprocessedThread,TblId FROM Threads")
    ...
    for row in cursor.fetchall():
        ...
        # print(row[0])
            ...
            if polarity == 'pos':
                ...
                cursor.execute("UPDATE Threads SET PolarityConfidence='"+str(confidencepos)+"' WHERE TblId =" + str(id))
                cursor.execute("UPDATE Threads SET ThreadPolarity='" + str(polarity) + "' WHERE TblId =" + str(id))
                cursor.commit()
            else:
                count1=count1+1
                confidenceneg=(-100)*confidence
                cursor.execute("UPDATE Threads SET PolarityConfidence='" + str(confidenceneg) + "' WHERE TblId =" + str(id))
                cursor.execute("UPDATE Threads SET ThreadPolarity='" + str(polarity) + "' WHERE TblId =" + str(id))
                # cursor.execute("UPDATE Threads SET Polarity='" +str(confidenceneg) + "' WHERE TblId =" + str(id))
                cursor.commit()
